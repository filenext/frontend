...
<template>
  <div class="row g-4">
    <div class="col-lg-7">
      <div class="card">
        <div class="card-header"><h3 class="card-title mb-0">品牌与主题</h3></div>
        <div class="card-body">
          <div class="row g-3">
            <div class="col-md-6">
            ...
            <div class="col-md-6">
              ...
              <input v-model="form.tagline" class="form-control" maxlength="64" placeholder="如：统一文件管理 · 智能体" />
            </div>
            <div class="col-md-6">
              <label class="form-label">Logo 文字（无图片时显示 1–2 字）</label>
              <input v-model="form.logo_text" class="form-control" maxlength="2" placeholder="留空则用默认图标" />
            </div>
            <div class="col-md-6">
            ...
            <div class="col-12">
              ...
              <textarea v-model="form.login_description" class="form-control" rows="2" maxlength="256" placeholder="显示在登录页品牌区，可介绍 AI 与智能体能力" />
            </div>
          </div>
          ...
          <div class="row g-3">
            <div class="col-md-6">
              ...
              <input v-model="form.footer_powered_by_name" class="form-control" maxlength="32" placeholder="NextFile" />
            </div>
            <div class="col-md-6">
              <label class="form-label">ICP 备案号</label>
              <input v-model="form.footer_icp" class="form-control" maxlength="64" placeholder="如：京ICP备1xx，留空则不显示" />
            </div>
            <div class="col-12">
              <p class="text-secondary small mb-0">
                  链接与微信已固定为 GitHub 仓库与 amwahaha，不可修改。
              </p>
            </div>
          </div>
          ...
          <div class="row g-3 mt-1">
            <div class="col-md-6">
              ...
              <input type="file" accept="image/*" class="form-control form-control-sm" @change="readImageFile($event, 'logo_url')" />
              <button v-if="form.logo_url" type="button" class="btn btn-sm btn-link px-0 mt-1" @click="clearImage('logo_url')">清除 Logo</button>
            </div>
            <div class="col-md-6">
              <label class="form-label">Favicon</label>
              <input type="file" accept="image/*" class="form-control form-control-sm" @change="readImageFile($event, 'favicon_url')" />
              <button v-if="form.favicon_url" type="button" class="btn btn-sm btn-link px-0 mt-1" @click="clearImage('favicon_url')">清除 Favicon</button>
            </div>
          </div>
        </div>
        <div class="card-footer d-flex gap-2">
          ...
          <button type="button" class="btn btn-sm" @click="loadForm">重置</button>
        </div>
      </div>
    </div>
    ...
    <div class="col-lg-5">
      <div class="card cd-preview-card overflow-hidden">
        ...
        <div class="card-body">
          ...
          <button type="button" class="btn btn-sm w-100" :style="{ background: form.primary_color, borderColor: form.primary_color, color: '#fff' }">
            主按钮示例
          </button>
        </div>
      </div>
