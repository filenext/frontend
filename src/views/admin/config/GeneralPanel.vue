...
const form = defineModel<{ page_size: number }>({ required: true })
...
async function save() {
  ...
  try {
    ...
    form.value = await settingsApi.saveSettings('general', { page_size: n })
    invalidatePageSize()
    toast.show('已保存')
  } catch (e) {
...
<template>
  <div class="cd-settings-panel">
    ...
    <p class="text-secondary small mb-4">全局列表分页与其它基础选项。</p>
    <div class="row g-3">
      <div class="col-md-4">
